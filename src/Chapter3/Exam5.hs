...
validateMarks :: InputScore -> [(StudentName, [(Subject, Marks, Bool, String)])]
validateMarks score =
    let validmarks = map (\(studentName,scorenames)->(studentName,             
                        concatMap(\(subjectname,MkMarks marks) ->
                            if (marks < 0)
                                then [(subjectname,MkMarks marks,False,"negative marks")]
                            else if (marks > 100)
                                then [(subjectname,MkMarks marks,False,"marks higher than 100")]
                            else if(subjectname `elem` validsubject)
                                then [(subjectname,MkMarks marks,True,"valid Subject")]
                            else [(subjectname,MkMarks marks,False,"unknown Subject")]  ) scorenames )) score
        in validmarks
...
averageMarks score subjectName =
    let markslist = validateMarksSubjects (\newlist (studentname,validstudent,subjectinfo) ->
                                newlist++(concatMap (\(subjectname,MkMarks marks,validsubject,validmessage)->
