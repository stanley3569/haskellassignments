module Chapter3.DateArithmeticNew5 where
...
leftInMonth (MkDate dd mm yy) = ( (monthLength mm yy) !! (mm-1)  )  - (dtDay (MkDate dd mm yy))     + 1     
...
addDays (MkDate dd mm yy) days =  
                            if ( days >= leftInYear (MkDate dd mm yy))
                                then addDays (MkDate dd 1 (yy+1)) (days- (leftInYear (MkDate dd mm yy)) )                            
...
dtDay :: Date -> Int
dtDay (MkDate day month year) = day


dtMonth :: Date -> Int
dtMonth (MkDate day month year) = month


dtYear :: Date -> Int
dtYear (MkDate day month year) = year
