module Chapter2.ExamWork1 where
import Data.List
import Data.Char
...
duplicateStudents score = 
    concat( filter (\x -> (length(x)>1 ) )  ( groupBy(\x y-> x==y ) (sort (fst (unzip(score))) ) )  ) 



standardDeviation :: InputScore -> ValidSubjects -> Subject -> Float          --2nd                           
standardDeviation score validsubject subjectName = 
    let averagevalue = snd(averageMarks score validsubject subjectName)
...
grouping score validsubject = 
                let listofstudent = studentDetails score validsubject                                               --1--[("Mojo",["English","Chemistry","Physics","Geography"]),("Captain Jack",["Chemistry","Geography"]),("Bahubali",[]),("Dexter",["Geography"])]  
