...
inputFile = do
    ...
    myscorefile <- ( BL.readFile scorefile)             --( BL.readFile scorefile)
...
data Result = Result { name :: !String, subject :: !String, marks :: !String}

instance FromNamedRecord Result where
    parseNamedRecord r = Result <$> r .: toS "name" <*> r .: toS "subject" <*> r .: toS "marks"
...
formatScore scoreString = 
    case (decodeByName scoreString) of
        Left err -> Left err
        Right (_,score1) -> 
            let scoreList =  DV.toList $ DV.map valuesToList score1
                grouping1 = DL.groupBy  (\a  x ->  (DL.head a) ==  (DL.head x ) ) (scoreList)        --(DV.toList scorelist) 
